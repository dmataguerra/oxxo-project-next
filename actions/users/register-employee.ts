...
export default async function registerEmployee(prevState: RegisterResult, formData : FormData): Promise<RegisterResult> {
    const userEmail = formData.get("userEmail");
    const userPassword = formData.get("userPassword");
    const employeeId = formData.get("employeeId");

    if (!employeeId) {
        return { success: false, error: "Falta employeeId en el formulario" };
    }
